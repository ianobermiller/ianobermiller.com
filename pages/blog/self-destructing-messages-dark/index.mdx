...
First, create a database to store the messages. I
started out very simple with this one, just added a `text`
field of type `String`. Later, I added a `createdAt` field
...
delete all the records first. That is easy enough, just add
a new repl with:
...
TODO: this works because Dark CORS default blah blah

Then in Dark, under 404s on the left side, you can select
the `/upload` endpoint and start coding:
...
Before saving the message, reject any requests that are
too long. The client also enforces this, but the client can
easily be bypassed by sending a request directly. The
checked length is actually greater than the reported max
because the client will encrypt and base64 encode, which
adds overhead.

Next, generate a database key (a UUID) using
...
You can verify that the database contains what you expect by
adding a new repl with:
...
Again, write a simple version of the client side code to
generate a trace to write the backend:
...
You can then find `/download` in the 404s section and fill it
out:
...
You might be wondering why I didn't go with a slightly more
RESTful API, with routes like `POST /message` and `GET /message/:id`. First, the download request
is not idempotent and shouldn't be a GET anyway. Second,
Dark works best if you have separate routes for each
endpoint, allowing you to more easily switch between traces
...
With the API feature complete, I filled in the UI. The [full
...
the creation confirmation screen.
...
First, we want the key (the part after the `=` sign) to be
in the hash of the url, since the hash it not sent to the
server. This gives you extra confidence that the server
...
how I deploy ianobermiller.com using Next.js. Next I
considered a url parameter, e.g.
...
bit number, we can display it however we want. Here, I use
the `uuid` library and url-safe base64 encoding to transform
the UUID given from Dark into a shorter, more friendly
looking random string:
...
For more details check out the [full source code on
GitHub](https://github.com/ianobermiller/ianobermiller.com/blob/5ee080fbd1a1359fbede7a0ec55270e363b3fd54/pages/secure/index.page.tsx).

## Conclusion

Overall this was a fun project to practice using Dark and
stand up a simple but useful service for myself. Future
